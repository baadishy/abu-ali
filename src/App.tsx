...
function AppContent() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        console.error(`Settings fetch failed (${res.status}):`, text.slice(0, 500));
        return;
      }
  ...
  const fetchMenuItems = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`Menu fetch failed (${response.status}):`, text.slice(0, 500));
        return;
      }
  ...
  const addToCart = (item: MenuItem, event?: React.MouseEvent, variantId?: string) => {
    if (event) {
      ...
      setFlies(prev => [...prev, { 
        ...
        x: rect.left, 
        y: rect.top, 
        targetX: cartRect ? cartRect.left : (isRTL ? 40 : window.innerWidth - 80),
      ...
      showNotification("success", isRTL ? "تمت الإضافة للسلة!" : "Added to Cart!");
    }

    setCart((prev) => {
      const existing = prev.find((i) => i._id === item._id && i.selectedVariantId === variantId);
      if (existing) {
        return prev.map((i) =>
          (i._id === item._id && i.selectedVariantId === variantId) ? { ...i, quantity: i.quantity + 1 } : i
        );
      }
  ...
  const removeFromCart = (id: string, variantId?: string) => {
    setCart((prev) => prev.filter((item) => !(item._id === id && item.selectedVariantId === variantId)));
  };
  ...
  const submitOrder = async (orderData: { name: string; phone: string; address: string; notes: string; deliveryFee: number; location: string; branchId: string; areaId?: string }) => {
    ...
    const whatsappNumber = rawWhatsapp.startsWith('2') ? rawWhatsapp : '2' + rawWhatsapp; 
    
    const { subtotal: rawSubtotal, discount, total: subtotalAfterDiscount, appliedOffers } = calculateOrderTotals(cart, settings, orderData.branchId);
    
    const isFreeDelivery = settings?.freeDeliveryThreshold && subtotalAfterDiscount >= settings.freeDeliveryThreshold;
    const finalDeliveryFee = isFreeDelivery ? 0 : orderData.deliveryFee;
    const total = subtotalAfterDiscount + finalDeliveryFee;

    try {
      const selectedBranch = settings?.branches?.find(b => b.id === orderData.branchId);
      const selectedArea = selectedBranch?.areas?.find(a => a.id === orderData.areaId);
      ...
      const response = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          items: cart.map(i => {
            const variant = i.variants?.find(v => v.id === i.selectedVariantId);
            const variantSuffix = variant ? ` (${language === "ar" ? variant.nameAr : variant.name})` : "";
            return { 
              name: i.name + variantSuffix, 
              price: variant ? variant.price : (i.discountPrice && i.discountPrice > 0 ? i.discountPrice : i.price), 
              quantity: i.quantity 
            };
          }),
    ...
    const customerPhone = orderData.phone.startsWith("2") ? orderData.phone : "2" + orderData.phone;
    
    let currentStoreName = language === "ar" ? (settings?.storeNameAr || settings?.storeName || t.brandName) : (settings?.storeName || t.brandName);
    ...
    if (orderData.notes) message += `📝 *${t.notesLabel}:* ${orderData.notes}\n`;
    message += `--------------------------\n`;
    ...
    cart.forEach((item) => {
      ...
      const variantName = variant ? ` (${language === "ar" ? variant.nameAr : variant.name})` : "";
  ...
  return (
    <div className={cn("min-h-screen bg-background text-white selection:bg-primary selection:text-black overflow-x-hidden", isRTL && "font-arabic")}>
      ...
      <main>
        <Hero settings={settings} featuredItem={featuredItem} onAddToCart={addToCart} isLoading={loading} />
        
        {settings?.offers && settings.offers.length > 0 ? (
          <PromotionalBanners offers={settings.offers} />
        ) : null}

        <section id="menu" className="max-w-screen-2xl mx-auto px-4 md:px-8 py-16 md:py-20 text-white">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 md:gap-8 text-[8px] md:text-[10px] uppercase font-black tracking-[0.2em] text-white/40 text-center md:text-left">
            ...
            <div className={cn("flex flex-wrap justify-center gap-4 md:gap-10 order-1 md:order-2", isRTL && "flex-row-reverse")}>
              {settings?.socialLinks.facebook && (
                <a href={settings.socialLinks.facebook} target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">{t.facebook}</a>
              )}
              {settings?.socialLinks.instagram && (
                <a href={settings.socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">{t.instagram}</a>
              )}
              {settings?.socialLinks.whatsapp && (
                <a href={`https://wa.me/${settings.socialLinks.whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-white flex items-center gap-2 hover:text-primary transition-colors">
